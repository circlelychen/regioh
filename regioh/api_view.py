...
def _extract_request_data(request):
    if request.form:
        ...
        mobile = jreq.get('mobile', None)
        token = jreq.get('token', None)
        otp = jreq.get('otp', None)
    if request.files and request.files.get('pubkey', None):
        fs_pubkey = request.files['pubkey']
        pub_key = fs_pubkey.read()
    return user_email, pub_key, mobile, token, otp

@app.route('/v1/check', methods=['POST'])
def v1_check():
    """Verify if the (email, public key, mobile#) is active/present"""
    user_email, pub_key, mobile, _, _ = _extract_request_data(request)
    if user_email is None or pub_key is None:
        abort(400, {'code': 400, 'message': 'missing email or public key'})
    if mobile is None:
        abort(400, {'code': 400, 'message': 'missing mobile number'})
    status, record = query_dynamodb(user_email, pub_key, mobile)
...
def v1_register():
    ...
    from api_helper import generate_OTP
    from api_helper import compute_C
    from api_helper import notify_email
    from api_helper import notify_sms
...
def v1_confirm():
    from binascii import hexlify
    user_email, pub_key, mobile, token, otp = _extract_request_data(request)
    status, record = query_dynamodb(user_email, token=hexlify(token), otp=otp)
    if status == u'active':
